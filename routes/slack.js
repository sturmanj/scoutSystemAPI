...
const viewGen = require("../viewGen.js")
...
slackApp.view("note-modal", async ({ ack, body }) => {
	await ack()
	const userId = body.user.id
	const teamNum =
		body["view"]["state"]["values"]["teamNum"]["static_select-action"]
			.selected_option.text.text
	const notes =
		body["view"]["state"]["values"]["notes"]["plain_text_input-action"]
			.value
	console.log(userId, teamNum, notes)
})
...
slackApp.event("app_home_opened", async ({ payload, client }) => {
	...
	for (let i = 0; i < matchList.length; i++) {
		blocks = blocks.concat(viewGen.matchView(i, matchList[i], userId))
	}
